        ...
        .then(data => {
            ...
            if (data != null && data.length > 0) {
                ...
                data.forEach((producto, index) => {
                    ...
                    <div class="form-floating">
                        <input type="number" class="form-control${producto.estatus ? "" : "-plaintext"} form-control-sm input-cantidad" id="cantidad-${index}" data-idProductoCompra="${producto.idProducto_Compra}" value="${producto.cantidad}" ${producto.estatus ? "" : "readonly"}>
                    ...
                    <div class="form-floating">
                        ...
                        <input type="number" class="form-control${producto.estatus ? "" : "-plaintext"} form-control-sm input-precioVenta" id="precioVenta-${index}" ${producto.estatus ? "" : "readonly"}>
                        <label for="precio-${index}">Precio Venta</label>
                    </div>
                    <div class="form-floating">
                ...
                // <div class="form-floating">
                //         <input type="number" class="form-control${producto.estatus ? "" : "-plaintext"} form-control-sm input-precioVenta" id="precioVenta-${index}" value="${producto.precioVenta}" ${producto.estatus ? "" : "readonly"}>
                //         <label for="precioVenta-${index}">Precio Venta</label>
                //     </div>
                ...
                btnGuardarCambios.addEventListener('click', (event) => {
                    ...
                    const inputsPrecioVenta = document.querySelectorAll('.input-precioVenta');

                    inputsCantidad.forEach((inputCantidad, index) => {
                        const idProductoCompra = inputCantidad.getAttribute('data-idProductoCompra');
                        const cantidad = inputCantidad.value;
                        const precio = inputsPrecio[index].value;
                        const precioVenta = inputsPrecioVenta[index].value;

                        let ProductoCompra = {
                            idProducto_Compra: idProductoCompra,
                            cantidad: cantidad,
                            precioCompra: precio,
                        ...
                        let queryString = {
                            datosProductoCompra: JSON.stringify(ProductoCompra)
                        };

                        const url = 'http://localhost:8080/api/producto_compra/guardar';
                        ...
                        fetch(url, {
                            ...
                            .then(data => {
                                ...
                                Swal.fire({
                                    ...
                                    text: "Los productos de la compra fueron actualizados correctamente.",
                                    // pasamos el parametro para el color de fondo del mensaje
                                    // se usa una verificación si isDarkMode es true el color de fondo es #333 si no es #fff
                                    background: isDarkMode ? '#333' : '#fff',
                                    // pasamos el parametro para el color del texto del mensaje
...
const cargarModalAgregar = () => {
    ...
    const carritoCompra = JSON.parse(sessionStorage.getItem('carritoCompra'));
    ...
    tituloModal.innerHTML = "Agregar Compra";
    // se limpia el contenido del modal
    contenidoModal.innerHTML = "";

    // se verifica si el carrito de ventas no es nulo y tiene al menos un producto
    if (carritoCompra != null && carritoCompra.length > 0) {
        // se imprime en consola los productos del carrito de ventas
        // console.log(carritoCompra);
        // se recorre cada producto del carrito de ventas
        carritoCompra.forEach((producto, index) => {
            // se agrega el codigo del producto y un input para la cantidad del producto
            productosModal += `
                            <div class="form-floating">
                                <input type="text" class="form-control-plaintext form-control-sm" id="codigoProductoAdd-${index}" value="${producto.codigoProducto}" readonly>
                                <label for="codigoProductoAdd-${index}">Codigo producto</label>
                            </div>
                            <div class="form-floating">
                                <input type="number" class="form-control form-control-sm input-cantidadAdd" id="cantidadAdd-${index}" data-codigoProducto="${producto.codigoProducto}" min="1" value="1">
                                <label for="cantidadAdd-${index}">Cantidad</label>
                            </div>
                            <div class="form-floating">
                                <input type="number" class="form-control form-control-sm input-precioAdd" id="precioAdd-${index}" min="1" value="1">
                                <label for="precioAdd-${index}">Precio Compra</label>
                            </div>
                            <div class="form-floating">
                                <input type="number" class="form-control form-control-sm input-precioVentaAdd" id="precioVentaAdd-${index}" min="1" value="1"}>
                                <label for="precioVentaAdd-${index}">Precio Venta</label>
                            </div>
                            `;
                });
        ...
        productosModal += `
                    <div class="text-center mt-3">
                        <button type="button" class="btn btn-success" id="btnGuardarCompra">Guardar Venta</button>
                    </div>
                `;
    } else {
    ...
    btnGuardarCompra.addEventListener('click', (event) => {
            ...
            .then(data => {
                ...
                inputsCantidad.forEach((inputCantidad, index) => {
                    ...
                    let ProductoCompra = {
                        ...
                        codigoProducto: inputCantidad.getAttribute('data-codigoProducto'),
                        // se obtiene la cantidad del producto del valor del input
                        cantidad: inputCantidad.value,
                        precioCompra: inputsPrecio[index].value,
                        precioVenta: inputsPrecioVenta[index].value
                    };
                    ...
                    const apiURL = 'http://localhost:8080/api/producto_compra/guardar';
                    ...
                    fetch(apiURL, {
                        ...
                        .then(res => {
                            ...
                            Swal.fire({
                                ...
                                text: "La compra con codigo "+data.codigoCompra+" fue agregada correctamente.",
                                // pasamos el parametro para el color de fondo del mensaje
                                // se usa una verificación si isDarkMode es true el color de fondo es #333 si no es #fff
                                background: isDarkMode ? '#333' : '#fff',
                                // pasamos el parametro para el color del texto del mensaje
